...
class CoMM(TransformerMixin, BaseEstimator):
    ...
    encoder : nn.Module or sequence of nn.Module
        Backbone applied to every modality, or several backbones assigned by
        `mod_to_encoder`.
    num_modalities : int
    ...
    enc_ckpt : str, sequence of str or None
        Path to CNN Lightning checkpoint(s) whose `encoder.` weights are loaded.
    freeze_encoder : bool, default=True
        Freeze the backbone (no grad, eval mode) and train only fusion + head.
    use_mod_embed : bool, default=False
        Add a learnable per-modality embedding to the fused tokens. Required
        for checkpoints pretrained before it became optional.
    ...
    **kwargs
    ...
    def _drop_object_hparams(self):
        """keep only plain-data hyper-parameters.
        """
