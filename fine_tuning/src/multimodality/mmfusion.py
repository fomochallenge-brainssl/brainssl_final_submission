...
class MissingModMMFusion(nn.Module):
    ...
    :param encoder: backbone, or list of backbones.
    ...
    :param use_mod_embed: add a learnable per-slot embedding to the scattered
        tokens.
    """
    def __init__(self,
        ...
        self.register_buffer(
            ...
            self._check_mod_to_encoder(mod_to_encoder, num_modalities,
                                       len(self.encoders)))
        if use_mod_embed:
            self.mod_embed = nn.Parameter(torch.zeros(num_modalities, embed_dim))
        else:
            self.register_parameter("mod_embed", None)
    ...
    def scatter(self, tokens, sample_idx, mod_idx, num_subjects):
        ...
        if self.mod_embed is not None:
            grid = grid + self.mod_embed[None, :, None, :]
        return grid, absent
