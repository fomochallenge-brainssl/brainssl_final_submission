...
## added: custom collate function for multimodal PretrainDataset
def comm_collate(batch):
    aug_keys = [k for k in batch[0] if k.startswith("aug")]
    out = {"num_subjects": len(batch)}
    out["sample_idx"] = torch.cat([
        torch.full((b["mod_idx"].numel(),), i, dtype=torch.long)
        for i, b in enumerate(batch)])
    out["mod_idx"] = torch.cat([b["mod_idx"] for b in batch])
    for k in aug_keys:
