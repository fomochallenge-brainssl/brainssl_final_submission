...
def build_3d_sincos_posemb(h, w, z, embed_dim=1024, temperature=10000.):
    """
    3d extension of 2d sin-cos positional embeddings from MoCo-v3
    """
    grid_w = torch.arange(w, dtype=torch.float32)
    grid_h = torch.arange(h, dtype=torch.float32)
    grid_z = torch.arange(z, dtype=torch.float32)
    grid_z, grid_h, grid_w = torch.meshgrid(grid_z, grid_h, grid_w, indexing='ij')
    ...
    out_w = torch.einsum('m,d->md', [grid_w.flatten(), omega])
    out_h = torch.einsum('m,d->md', [grid_h.flatten(), omega])
    out_z = torch.einsum('m,d->md', [grid_z.flatten(), omega])
    pos_emb = torch.cat([torch.sin(out_w), torch.cos(out_w), torch.sin(out_h), torch.cos(out_h), torch.sin(out_z), torch.cos(out_z)], dim=1)[None, :, :]
    pos_emb = rearrange(pos_emb, 'b (z h w) d -> b d z h w', z=z, h=h, w=w, d=embed_dim)
    return pos_emb
...
class IdentityAdapter(nn.Module):
    """
        Adapter for transformer encoders, whose output is already a sequence of tokens
    """
    ...
    def forward(self, x):
        ...
        assert x.ndim == 3, f"Expected 3D tensors of shape (B, N, E), got {x.shape}"
        return x
...
class Patched3DInputAdapter(nn.Module):
    """Adapter for 3D spatial inputs, like volumetric images or feature maps.
    Creates tokens from patches over the volume.

    :param num_channels: Number of input channels of the volume
    :param stride_level: Stride level compared to the full-sized volume.
        E.g. 4 for 1/4th the size of the volume.
    :param patch_size_full: Int or tuple of the patch size over the full volume size.
        Patch size for smaller inputs will be computed accordingly.
    :param dim_tokens: Dimension of output tokens. Can be set using init method.
    :param sincos_pos_emb: Set to True (default) to use fixed 3D sin-cos positional embeddings
    :param learnable_pos_emb: Set to True to learn positional embeddings instead
    :param volume_size: Default volume size. Used to initialize size of positional embeddings.

    Volumes are laid out as (B, C, Z, H, W), matching Conv3d and the
    (b, d, z, h, w) layout of `build_3d_sincos_posemb`.
    """
    def __init__(self,
                 num_channels: int,
                 stride_level: int,
                 ...
                 sincos_pos_emb: bool = True,
                 learnable_pos_emb: bool = False,
                 volume_size: Union[int, Tuple[int, int, int]] = 128):

        super().__init__()
        self.num_channels = num_channels
        self.stride_level = stride_level
        self.patch_size_full = triple(patch_size_full)
        self.dim_tokens = dim_tokens
        self.sincos_pos_emb = sincos_pos_emb
        self.learnable_pos_emb = learnable_pos_emb
        self.volume_size = triple(volume_size)
        self.num_patches = ((self.volume_size[0] // self.patch_size_full[0])
                            * (self.volume_size[1] // self.patch_size_full[1])
                            * (self.volume_size[2] // self.patch_size_full[2]))

        # Actual patch size per axis, taking the stride of the input into account
        self.P_Z = max(1, self.patch_size_full[0] // stride_level)
    ...
    def init(self, dim_tokens: int = 768):
        """
        Initialize parts of the adapter that depend on the dimension of tokens.
        :param dim_tokens: Dimension of tokens
        """
        self.dim_tokens = dim_tokens

        # Fixed-size positional embeddings, interpolated to the actual grid in forward
        z_posemb = self.volume_size[0] // (self.stride_level * self.P_Z)
        h_posemb = self.volume_size[1] // (self.stride_level * self.P_H)
        w_posemb = self.volume_size[2] // (self.stride_level * self.P_W)
        if self.sincos_pos_emb:
            assert self.dim_tokens % 6 == 0, (
                f"dim_tokens must be divisible by 6 for 3D sin-cos position "
                f"embeddings (2 functions x 3 axes), got {self.dim_tokens}")
            self.pos_emb = build_3d_sincos_posemb(
                h=h_posemb, w=w_posemb, z=z_posemb, embed_dim=self.dim_tokens)
            self.pos_emb = nn.Parameter(self.pos_emb, requires_grad=self.learnable_pos_emb)
        else:
            self.pos_emb = nn.Parameter(
                torch.zeros(1, self.dim_tokens, z_posemb, h_posemb, w_posemb))
            trunc_normal_(self.pos_emb, std=0.02)

        # Volume -> tokens projection
        self.proj = nn.Conv3d(
            in_channels=self.num_channels, out_channels=self.dim_tokens,
            kernel_size=(self.P_Z, self.P_H, self.P_W),
            stride=(self.P_Z, self.P_H, self.P_W)
        )
    ...
    def forward(self, x):
        """
        Forward pass through the input adapter, transforming a volume into a
        sequence of tokens and adding positional encodings.

        :param x: Input volume tensor of shape (B, C, Z, H, W)
        :return: Tokens of shape (B, N_Z * N_H * N_W, dim_tokens)
        """
        B, C, Z, H, W = x.shape
        assert self.dim_tokens is not None, 'Need to call init(dim_tokens) function first'
        assert (Z % self.P_Z == 0) and (H % self.P_H == 0) and (W % self.P_W == 0), (
            f'Volume sizes {Z}x{H}x{W} must be divisible by patch sizes '
            f'{self.P_Z}x{self.P_H}x{self.P_W}')
        # Number of patches along each axis
        N_Z, N_H, N_W = Z // self.P_Z, H // self.P_H, W // self.P_W

        # Create patches [B, C, Z, H, W] -> [B, (N_Z*N_H*N_W), D]
        x_patch = rearrange(self.proj(x), 'b d nz nh nw -> b (nz nh nw) d')

        # Create positional embedding
        x_pos_emb = F.interpolate(self.pos_emb, size=(N_Z, N_H, N_W),
                                  mode='trilinear', align_corners=False)
        x_pos_emb = rearrange(x_pos_emb, 'b d nz nh nw -> b (nz nh nw) d')
